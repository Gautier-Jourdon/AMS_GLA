 ...
 * Gestion de l'envoi d'emails via SMTP et Discord
 */
...
export async function sendDiscordNotification(webhookUrl, message) {
  if (!webhookUrl || !webhookUrl.startsWith('http')) {
    logger.warn(`[DISCORD] Invalid Webhook URL provided`);
    return;
  }
  try {
    await axios.post(webhookUrl, {
      content: message,
      username: "AMS Crypto Bot",
      avatar_url: "https://i.imgur.com/4M34hi2.png"
    });
    logger.info(`[DISCORD] Notification sent to webhook`);
  } catch (err) {
    logger.error(`[DISCORD] Failed to send notification: ${err.message}`);
  }
}
  ...
  try {
    ...
    const html = `
        ...
        <head>
            <meta charset="UTF-8">
            <style>
                body { font-family: -apple-system, sans-serif; background: #0a0e1a; color: #e6eef8; padding: 20px; }
                .container { max-width: 600px; margin: 0 auto; background: #1a1f2e; border: 1px solid rgba(0,212,255,0.2); border-radius: 12px; padding: 30px; }
                h2 { color: #00d4ff; margin-top: 0; }
                .btn { display: inline-block; padding: 12px 24px; background: #00d4ff; color: #fff; text-decoration: none; border-radius: 6px; margin: 20px 0; }
            </style>
        </head>
        <body>
            <div class="container">
                <h2>🔔 Confirmation d'Alerte</h2>
                <p>Vous avez créé une alerte pour <strong>${alert.symbol}</strong>.</p>
                <p>Seuil: $${alert.threshold} (${alert.direction})</p>
                <a href="${confirmUrl}" class="btn">Confirmer</a>
            </div>
        </body>
        </html>`;
    ...
    const result = await transporter.sendMail({
      ...
      html,
      text: `Confirmez votre alerte pour ${alert.symbol}: ${confirmUrl}`
    });
...
export async function sendAlertTriggered(userEmail, alert, currentPrice) {
  ...
  // Check for Discord delivery
  if (alert.delivery_method === 'discord' && userEmail.startsWith('http')) {
    const msg = `🔔 **ALERTE DÉCLENCHÉE** 🔔\n\nCrypto: **${alert.symbol}**\nPrix Actuel: **$${currentPrice}**\nSeuil: **$${alert.threshold}**\nDirection: ${alert.direction === 'above' ? '📈 Au-dessus' : '📉 En-dessous'}`;
    await sendDiscordNotification(userEmail, msg);
  ...
  try {
    ...
    const html = `
        ...
        <body style="background:#0a0e1a; color:#fff; padding:20px; font-family:sans-serif;">
            <div style="max-width:600px; margin:0 auto; background:#1a1f2e; padding:30px; border-radius:10px; border:2px solid ${alert.direction === 'above' ? '#00ff88' : '#ff4757'};">
                <h2 style="color:${alert.direction === 'above' ? '#00ff88' : '#ff4757'}">Alerte Déclenchée !</h2>
                <p><strong>${alert.symbol}</strong> est à <strong>$${priceFormatted}</strong></p>
                <p>Seuil: $${alert.threshold}</p>
            </div>
        </body>
        </html>`;
    ...
    const result = await transporter.sendMail({
      from: process.env.EMAIL_FROM || '"AMS GLA Alerts" <no-reply@ams-gla.com>',
      to: userEmail,
      subject: `ALERTE ${alert.symbol}: $${priceFormatted}`,
      html,
      text: `ALERTE ${alert.symbol} est à $${priceFormatted}`
    });

    logger.info('[EMAIL] Alert triggered sent', { to: userEmail });
    return { success: true, messageId: result.messageId };

  } catch (error) {
    logger.error('[EMAIL] Failed to send triggered alert', { error: error.message });
    throw error;
  }
}

export async function testSmtpConfig() {
  if (!process.env.SMTP_HOST) return { success: false, error: 'SMTP_HOST not configured' };
  try {
    const transporter = createTransporter();
